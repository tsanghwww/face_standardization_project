    ...
    if type == 'pytorch3d':
        ...
        standard_rasterize_cuda = \
            load(name='standard_rasterize_cuda', 
                ...
                extra_cuda_cflags=['-std=c++17', '--allow-unsupported-compiler'])
...
class Pytorch3dRasterizer(nn.Module):
    ...
    def forward(self, vertices, faces, attributes=None, h=None, w=None):
        output_device = vertices.device
        raster_device = torch.device('cpu') if output_device.type == 'mps' else output_device
        ...
        fixed_vertices = fixed_vertices.to(raster_device)
        faces = faces.to(raster_device)
        attributes = attributes.to(raster_device)
        ...
        pixel_vals = pixel_vals[:,:,:,0].permute(0,3,1,2)
        pixel_vals = torch.cat([pixel_vals, vismask[:,:,:,0][:,None,:,:]], dim=1)
        pixel_vals = pixel_vals.to(output_device)
        # print(image_size)
