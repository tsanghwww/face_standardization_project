...
class DECA(nn.Module):
    def __init__(self, config=None, device='cuda', render_enabled=True):
    ...
    def decode(self, codedict, rendering=True, iddict=None, vis_lmk=True, return_vis=True, use_detail=True,
        ...
        ## rendering
        if rendering and return_vis and render_orig and original_image is not None and tform is not None:
        ...
        if rendering:
            if not self.render_enabled:
                raise RuntimeError('Rendering was requested, but DECA was created with render_enabled=False.')
