...
def extract_param_outputs(codedict):
    param_keys = {
        'shape': 'shape',
        'exp': 'expression',
        'pose': 'pose',
    ...
    params = {}
    for source_key, output_key in param_keys.items():
        if source_key not in codedict:
            continue
        value = codedict[source_key]
...
def main(args):
    ...
    if not args.rendering:
        ...
        args.render_orig = False
    deca = DECA(config = deca_cfg, device=device, render_enabled=args.rendering)
    ...
    for i in tqdm(range(len(testdata))):
        ...
        with torch.no_grad():
            ...
            decoded = deca.decode(codedict, rendering=args.rendering, return_vis=args.rendering) #tensor
            if args.rendering:
                opdict, visdict = decoded
            else:
                opdict = decoded
                visdict = {}
            if args.rendering and args.render_orig:
        ...
        if args.saveMat:
            ...
            savemat(os.path.join(savefolder, name, name + '.mat'), matdict)
        if args.saveVis:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--device', default='auto', type=str,
                        help='set device: auto, cuda, mps, or cpu' )
    # process test images
    ...
    parser.add_argument('--rendering', default=True, type=lambda x: x.lower() in ['true', '1'],
                        help='whether to run the mesh renderer. Set false when pytorch3d/CUDA rasterizers are unavailable.')
    # save
