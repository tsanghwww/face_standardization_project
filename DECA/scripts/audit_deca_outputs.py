...
def check_kpts(kpt2d_path: Path, kpt3d_path: Path, image_size: int, reasons: list[str], t: Thresholds) -> None:
    ...
    xy = k2[:, :2]
    out = (
        (xy[:, 0] < 0) | (xy[:, 0] > image_size) |
        (xy[:, 1] < 0) | (xy[:, 1] > image_size)
    )
...
def audit_one(input_path: Path, results_dir: Path, profile: str, image_size: int, t: Thresholds) -> dict[str, str]:
    ...
    if paths["kpt2d"].exists() and paths["kpt3d"].exists():
        check_kpts(paths["kpt2d"], paths["kpt3d"], image_size, reasons, t)
    if paths["mat"].exists():
...
def main() -> None:
    ...
    rows = [audit_one(p, args.results_dir, args.profile, args.image_size, thresholds) for p in images]
